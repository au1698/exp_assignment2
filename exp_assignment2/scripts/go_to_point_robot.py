...
import math
import actionlib  # library action
...
def change_state(state):
    global state_
    state_ = state
    print('State robot changed to [%s]' % state_)
...
def fix_yaw(des_pos):
    ...
    #rospy.loginfo('fix yaw robot')

    # state change conditions
    if math.fabs(err_yaw) <= yaw_precision_2_:
        print('Yaw error robot: [%s]' % err_yaw)
        change_state(1)
        rospy.loginfo('state robot changed to 1')
...
def planning(goal):
    ...
    rospy.loginfo('Goal arrived to the robot server ')

    # when the goal arrives, set the desired position
    ...
    # a sort of state machine is implemented
    while not rospy.is_shutdown():
        # check is the client cancels the current goal
        if act_s.is_preempt_requested():  
            rospy.loginfo('Goal robot of robot was preempted')
            act_s.set_preempted()
            success = False
            break   
        elif state_ == 0:
            ...
            feedback.position = pose_
            act_s.publish_feedback(feedback)
            rospy.loginfo('state robot is reached')
            done()
            break
        else:
        ...
        rate.sleep()
    if success:
        rospy.loginfo('Goal robot: Succeeded!')
